...
const SYSTEM = `أنت محرر حساب AZ Alpha Vision. اكتب منشور X عربيًا مختصرًا عن حدث حقيقي في محاكي تداول افتراضي تعليمي. اذكر أن المحاكي افتراضي وتعليمي، ولا تقدم توصية شراء أو بيع أو وعدًا بالربح. لا تخترع أرقامًا أو أخبارًا. استخدم مصدر الحدث إذا توفر. اكتب نصًا قصيرًا لا يتجاوز 150 حرفًا قبل أن يضيف النظام رابط التسجيل والمصدر. لا تضع روابط أو هاشتاقات بنفسك. لا تستخدم لغة جازمة عن اتجاه السعر.`;
async function generateEventPost(event) {
  const p = event.payload;
  let prompt = '';
  if (event.eventType === 'news') prompt = `خبر مهم للسهم ${event.symbol}: ${p.title}. التصنيف: ${p.category}. الأثر المحتمل: ${p.impact}. المصدر: ${event.sourceUrl || ''}`;
