...
async function askAzAi(event) {
  ...
  const loading = appendAzAiMessage("assistant", "AZ ai يكتب...");
  ...
  try {
    ...
    const { data, error } = await sb.functions.invoke("az-ai", {
      body: { messages: azAiHistory },
    });
    if (error) throw new Error(error.message || "تعذر الاتصال بالمساعد");
    if (!data?.answer)
      throw new Error(data?.error || "لم يصل رد صالح من AZ ai");
    loading?.remove();
    appendAzAiMessage("assistant", data.answer);
    ...
    appendAzAiMessage(
      "assistant",
      `تعذر تشغيل AZ ai حاليًا. تحقق من نشر الوظيفة ومفتاح الذكاء الاصطناعي في Supabase.${detail ? `\nالتفصيل: ${detail}` : ""}`,
    );
    console.error(error);
  }
}
...
const PLATFORM_GUIDE = {
  ...
  support: {
    title: "الدعم الفني",
    body: "<p>استخدم الدعم لإرسال ملاحظة أو مشكلة عن الحساب أو البيانات أو تجربة المحاكي، وستصل كتذكرة للمسؤول.</p>",
  },
};
