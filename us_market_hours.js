 ...
 * الجلسات المسموحة للتداول في المحاكي:
 *   قبل التداول 04:00–09:30 | الجلسة الرسمية 09:30–16:00 | بعد التداول 16:00–20:00
 * في أيام الإغلاق المبكر: الجلسة حتى 13:00 ثم بعد التداول حتى 17:00.
 * وقوف تام: عطلة نهاية الأسبوع، الإجازات الرسمية، وخارج الساعات الممتدة.
 */
(function (root, factory) {
  const api = factory();
  if (typeof module === "object" && module.exports) module.exports = api;
  if (root) root.AzUsMarketHours = api;
  ...
  function nyseCalendar(year) {
    ...
    const add = (parts, name) => holidays.set(ymdKey(parts.year, parts.month, parts.day), name);
    ...
    add(shiftDays(easter, -2), "الجمعة العظيمة");
    ...
    if (eveWd !== 0 && eveWd !== 6) addEarly(christmasEve);
    const jul3 = { year, month: 7, day: 3 };
    const jul3Wd = new Date(Date.UTC(year, 6, 3)).getUTCDay();
    if (jul3Wd !== 0 && jul3Wd !== 6) addEarly(jul3);
  ...
  function lookupInYears(ymd, year, field) {
    for (const y of [year - 1, year, year + 1]) {
      const cal = nyseCalendar(y);
      if (field === "holiday" && cal.holidays.has(ymd)) return cal.holidays.get(ymd);
      if (field === "early" && cal.earlyCloses.has(ymd)) return true;
    }
    return field === "holiday" ? null : false;
  }
  ...
  function getUsMarketClock(date) {
    const now = date instanceof Date ? date : new Date();
    const ny = nyParts(now);
    const weekend = ny.weekday === "Sat" || ny.weekday === "Sun";
    const holidayName = weekend ? null : lookupInYears(ny.ymd, ny.year, "holiday");
    const earlyClose = !weekend && !holidayName && lookupInYears(ny.ymd, ny.year, "early");
  ...
  return { getUsMarketClock, nyseCalendar, TZ };
});
